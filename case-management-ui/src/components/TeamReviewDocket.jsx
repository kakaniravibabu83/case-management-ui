...
export default function TeamReviewDocket({
  tasks = [],
  selectedTaskId,
  onSelectTask,
  currentUser,
  groupName = "GROUP_REVIEW",
  teamTitle = "Review Tasks",
  loading = false,
  onRefresh
}) {
  ...
  return (
    <aside
      className="docket docket--biz-queue docket--team-queue"
      aria-label={`${teamTitle} Task Queue`}
    >
      <div className="docket__header">
        <div className="docket__header-row">
          <p className="docket__eyebrow">Queue • {groupName}</p>
        ...
        <h1 className="docket__title">{teamTitle}</h1>
      </div>
      ...
      >
        <button
          type="button"
          className={`docket__chip ${
            filterTab === "all" ? "docket__chip--active" : ""
          }`}
        ...
        <button
          type="button"
          className={`docket__chip ${
            filterTab === "unassigned" ? "docket__chip--active" : ""
          }`}
        ...
        <button
          type="button"
          className={`docket__chip ${
            filterTab === "mine" ? "docket__chip--active" : ""
          }`}
      ...
      {loading && tasks.length === 0 ? (
        ...
        <p className="docket__empty">
          {filterText || filterTab !== "all"
            ...
            : `No ${teamTitle} tasks assigned to queue yet.`}
        </p>
      ) : (
